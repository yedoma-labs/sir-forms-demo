...
import { createFormAction, schema, parseFormData } from '@yedoma-labs/suruy-form-actions'
import { z } from 'zod'
...
export const hybridProcessCheckout = createFormAction(
  async (formData) => {
    ...
    const expYear = Number.parseInt(result.data.expiryYear)
    const expMonth = Number.parseInt(result.data.expiryMonth)
...
export const hybridFinalSubmit = createFormAction(
  async (formData) => {
    ...
    actionLogger.debug('Final submit - received data', { data, action: 'hybridFinalSubmit' })
    ...
    if (!step1Result.success || !step2Result.success) {
      ...
      actionLogger.warn('Multi-step validation failed', { errors, action: 'hybridFinalSubmit' })
