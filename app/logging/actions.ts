...
export async function testLogLevels(): Promise<string> {
  ...
  logger.debug('This is a DEBUG message', { level: 'debug', timestamp: Date.now() })
  output += '✅ DEBUG: Detailed diagnostic information\n'
  
  logger.info('This is an INFO message', { level: 'info', event: 'user_action' })
  output += '✅ INFO: General informational messages\n'
  
  logger.warn('This is a WARN message', { level: 'warn', warning: 'low_disk_space' })
  output += '✅ WARN: Warning messages for potential issues\n'
  
  logger.error('This is an ERROR message', { level: 'error', errorCode: 'E001' })
...
export async function testChildLoggers(): Promise<string> {
  ...
  const dbLogger = logger.child({ module: 'database', service: 'user-service' })
  ...
  dbLogger.info('Database query executed', { query: 'SELECT * FROM users', duration: 23 })
  ...
  const userAuthLogger = authLogger.child({ feature: 'login', userId: 'user_456' })
