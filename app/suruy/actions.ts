...
export const suruyQuickSubscribe = createSimpleAction(async (formData) => {
  ...
  actionLogger.info('Newsletter subscription', { email, action: 'suruyQuickSubscribe' })
...
export const suruyAddTags = createSimpleAction(async (formData) => {
  ...
  if (!category) {
    return {
      success: false,
      errors: { category: ['Category is required'] } as Record<string, string[]>,
    }
  }
  ...
  if (tags.length === 0) {
    return {
      success: false,
      errors: { tags: ['At least one tag is required'] } as Record<string, string[]>,
    }
  }

  await new Promise((resolve) => setTimeout(resolve, 600))

  actionLogger.info('Tags added', { category, tags, tagCount: tags.length, action: 'suruyAddTags' })
